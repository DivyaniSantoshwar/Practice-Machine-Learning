...
boston = pd.read_csv("C:/Users\dai\Desktop\Machine Learning\Datasets\Boston.csv")
y = boston['medv']
...
##########
voting = VotingRegressor([('LR',lr),('RIDGE',ridge),('TREE',dtr),
                           ('LASSO',lasso)],weights=[r2_lr,r2_ridge,r2_dtr,
                                                     r2_lasso])
voting.fit(X_train,y_train)
y_pred = voting.predict(X_test)
r2_voting = r2_score(y_test, y_pred)                   
...
voting_weighted = VotingRegressor([('LR',lr),('RIDGE',ridge),('DTR',dtr),
                           ('LASSO',lasso)],weights=[r2_lr,r2_ridge,r2_dtr,
                                                     r2_lasso])
gcv_weighted = GridSearchCV(voting, param_grid=params, cv=kfold ,n_jobs=-1)
gcv_weighted.fit(X, y)                                                     
#r2_voting = r2_score(y_test, y_pred)   
print(gcv_weighted.best_params_)
print(gcv_weighted.best_score_)    
...
params={'TREE__min_samples_split':[2,4,5,8,10],
...
rgcv = RandomizedSearchCV(voting, param_distributions=params,random_state=24,
                          cv=kfold
